...
## Overview
In this part, we'll simulate signals from an image. Let me give some more context. When we mean signal simulation from an image, we mean an inverse approach where we try to simulate signals received by a uniform linear array if the image represents the amplitude of scatters. 

## Setup

<!-- Scatter positioning parameters -->
The first step in setting up this signal simulation pipeline is to choose the physical bounds of the scatterers. These are represented by the parameters: 
- top-left coordinate 
- bottom-right coordinate. 

These coordinates corresponds to the coordinates of the pixels in the top-left and bottom-right. The code then assigns the coordinates of the rest of the pixels using this information. 

Similarly, this is followed by setting up the position of the sensor array. The sensor array parameters are as follows
- *sensor array origin*: where the first sensor is placed. 
- *sensor spacing*: the arrray we're using is a uniform linear array, which means that the sensors are equally spaced. 
- *sensor-array direction*: the sensor array direction corresponds to the vector pointing from the first sensor the rest of the sensors. Please note that the director vector is normalized. 
- *emitter location*: Corresponds to the location of the SONAR signal emitter. 


In addition to the physical parameters, we also need to setup the signal parameters. The signal parameters are as follows

- *signal speed*: speed of sound in water
- *signal frequency*: frequency of signal emitted. Currently, we're using a uniform signal but note that usually, chirp is used. 
- *recording time*: the amount of time the hydrophones are turned on to listen after the signal is emitted. This parameter defines the range of the system. 
...
Next, we define the sensor-array parameters that will be used to simulate signals that arrive at the sensors of the signal array. We primarily setup the following parameters
- *sensor array origin*: where the first sensor is placed. 
- *sensor spacing*: the arrray we're using is a uniform linear array, which means that the sensors are equally spaced. 
- *sensor-array direction*: the sensor array direction corresponds to the vector pointing from the first sensor the rest of the sensors. Please note that the director vector is normalized. 
- *emitter location*: Corresponds to the location of the SONAR signal emitter. 
...
Next, we setup the parameters of the signals used in this setup. 
- *signal speed*: speed of sound in water
- *signal frequency*: frequency of signal emitted. Currently, we're the chirp frequency. The initial frequency of the sweep is this value and the maximum value is the half of sampling frequency. 
- *recording time*: the amount of time the hydrophones are turned on to listen after the signal is emitted. This parameter defines the range of the system. 
- *transmission duration*: This is the length of the emitted signal.
- *max range*: The max range is the range of scan. It depends on the recording time. 
...
Next, we build the image that is used to represent the scatterers. We use the inbuilt Matlab function, *phantom*, of size 1024. We then assign the physical bounds of the scatterers using 
- *top-left coordinate*: coordinate of the top-left pixel
- *bottom-right coordinate*: coordinate of the bottom-right pixel. 
...
Next, we call the script that does the signal simulation. 
```Matlab
...
Next, we plot the resulting signal matrix. 
...
As usual, we start by putting together two definitions: the gateway function definition and kernel-definition. 
...
<!--  -->
### Gateway Function Definition

#### Final gateway function definition
```C
// main function
